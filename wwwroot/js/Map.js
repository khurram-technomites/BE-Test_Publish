...
function getLocation01(branchElem = "") {
    debugger;
    if (branchElem != "") {
        ChangeRoute = true;
        elem = $(branchElem);
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(showPosition01);
        IsGetCurrentLocation = true;
    } else {
        alert("Geolocation is not supported by this browser.");
        ...
        latitude01 = 25.2048
        longitude01 = 55.2708
        InsertPosition01(latitude01, longitude01);
        InsertLatLonInInput01(latitude01, longitude01);
    }
}
...
function showPosition01(position) {

    latitude01 = position.coords.latitude01;
    longitude01 = position.coords.longitude01;

    InsertLatLonInInput(latitude01, longitude01);
    InsertPosition(latitude01, longitude01);
...
function InsertPosition01(latitude01, longitude01) {
    $.ajax({
        type: 'Get',
        url: `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=AIzaSyD96pSIJVVnH9RDyy-n2G3uu3FG8Ze9xyc`,
        success: function (response) {
            if (response.results.length > 0) {
                ...
                if (IsGetCurrentLocation) {
                    SetCurrentLocation(latitude01, longitude01, MapAddress);
                    IsGetCurrentLocation = false;
                }
...
function InsertLatLonInInput01(latitude01, longitude01) {
    if (ChangeRoute) {
        debugger;
        $(elem).closest(".location-div").find(".Latitude01").val(latitude01);
        $(elem).closest(".location-div").find(".Longitude01").val(longitude01);
    }
    else {

        $('#Latitude01').val(latitude01);
        $('#Longitude01').val(longitude01);
    }
}
...
function InsertMapAddressInInput01(MapAddress) {
    if (ChangeRoute) {
        $(elem).closest(".location-div").find(".branch-Address").val(MapAddress);
        $('#current-location01').val(MapAddress);
    }
    else {
        $('#Address01').val(MapAddress);
        $('#current-location01').val(MapAddress);
    }
}
...
function openMap(element = null) {
    ...
    if (!IsMapLoaded) {
        ...
        myMap();
        InsertMapAddressInInput01(MapAddress)
    }
    $('#map-modal').modal('show');
}
