...
$(document).on("change", ".fileUpload", function (e) {
    var Count = 0
    if (this.files[0].name.includes('pdf') == true || this.files[0].name.includes('doc') == true || this.files[0].name.includes('rar') == true || this.files[0].name.includes('zip') == true) {
        
       
        var currentdate = new Date();
        filePath = UploadImageToDraft(this.files[0]).responseText;
        var type = this.dataset.type;
        if (type == "Picture Gallery" && this.files[0].size/1024/1024 > 25) {
            toastr.error('25 Mb file size allowed')
            return false
        }
        Content = {
            DocumentType: type,
        ...
        toastr.error('Only doc, pdf, archive format allowed')
        return false
    }
