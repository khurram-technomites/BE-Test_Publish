            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        var actions = '';
                        actions += '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Client/NewsAndEvents/Edit?Id=' + data + '\')" title="View">' +
                            '<i class="fa fa-pen"></i>' +
                            '</button> ' +
                            '<button type="button" class="btn btn-bg-secondary btn-icon btn-sm mr-1" onclick="OpenModelPopup(this,\'/Client/NewsAndEvents/Details/' + data + '\')" title="View">' +
...
function Delete(element, record) {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Client/NewsAndEvents/Delete/' + record,
                type: 'POST',
                data: {
...
function Activate(element, record) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                url: '/Client/NewsAndEvents/ToggleActiveStatus/' + record,
                type: 'Get',
...
function addRow(row) {
    ...
    table1.row.add($('<tr>' +
        '<td data-order=' + row.id + '>' + row.posteddate + '</td >' +
        '<td>' + row.title + '</td>' +
        '<td>' + row.status + '</td>' +
